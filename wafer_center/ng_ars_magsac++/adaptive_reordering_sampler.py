...
class AdaptiveReorderingSampler:
    def __init__(self, points, m=3, a=1, b_init=1, threshold=0.5, max_iterations=2000):
        self.points = points
        self.m = m
        self.a = a
        self.b_init = b_init
        ...
        self.mu = np.ones(n_points) * 0.2  # 初始概率
...
ars = AdaptiveReorderingSampler(points, threshold=0.11, max_iterations=1000)
ars_model, ars_inliers = ars.fit()
# 记录结束时间
end_time = time.time()
# 计算耗时，单位转换为毫秒
elapsed_time = (end_time - start_time) * 1000
print(f"算法耗时: {elapsed_time} 毫秒")
